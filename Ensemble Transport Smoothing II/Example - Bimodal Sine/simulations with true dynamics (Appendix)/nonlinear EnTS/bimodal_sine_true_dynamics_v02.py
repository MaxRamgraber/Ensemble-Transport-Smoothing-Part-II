if __name__ == '__main__':    
    ...
    for mode in [0,1,2]:
        ...
        # Label the axes
        plt.title("linear ensemble transport smoother (based on nonlinear filter)",fontsize=10)
        plt.ylabel("state $x^{a}$")
        ...
        plt.title("nonlinear ensemble transport smoother (based on nonlinear filter)",fontsize=10)
        plt.ylabel("state $x^{a}$")
        plt.xlabel("time step")
        ...
        # Save the figure
        plt.savefig('bimodal_smoother_true'+string+'.png',dpi = 300,bbox_inches='tight')

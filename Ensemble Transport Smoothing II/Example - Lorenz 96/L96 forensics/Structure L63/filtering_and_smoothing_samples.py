...
if "synthetic_truth_L63.p" not in list(os.listdir(root_directory)) or \
   "observations_L63.p" not in list(os.listdir(root_directory)):
    ...
    observations        = copy.copy(synthetic_truth[:,:])
    ...
    for t in np.arange(1,T,1):
        ...
        observations[t,:]   = copy.copy(synthetic_truth[t,:])
    ...
    pickle.dump(synthetic_truth,    open("synthetic_truth_L63.p","wb"))
    pickle.dump(observations,       open("observations_L63.p","wb"))
    ...
    synthetic_truth     = pickle.load(open("synthetic_truth_L63.p","rb"))
    observations        = pickle.load(open("observations_L63.p","rb"))
...
# If we haven't precalculated observations and truth, do so.
if "Z_f_L63.p" not in list(os.listdir(root_directory)) or \
   "Z_a_L63.p" not in list(os.listdir(root_directory)):
    ...
    for t in np.arange(0,T,1):
        ...
        Y[t,...]    = copy.copy(Z_f[t,...][:,:]) + scipy.stats.norm.rvs(
            scale   = obs_sd,
            size    = Z_f[t,...][:,:].shape)
...
for s in range(100):
    ...
    if "ksd_L63_smoother_t="+str(t).zfill(3)+".p" not in list(os.listdir(root_directory)):
        ...
        # Store the results
        pickle.dump(ksds_s,     open("ksd_L63_smoother_t="+str(t).zfill(3)+".p",          "wb"))
        pickle.dump(ksds_marg_s,open("ksd_L63_smoother_marginal_t="+str(t).zfill(3)+".p", "wb"))
    ...
    if "ksd_L63_filter_t="+str(t).zfill(3)+".p" not in list(os.listdir(root_directory)):
        ...
        # Store the results
        pickle.dump(ksds_f,     open("ksd_L63_filter_t="+str(t).zfill(3)+".p",            "wb"))
        pickle.dump(ksds_marg_f,open("ksd_L63_filter_marginal_t="+str(t).zfill(3)+".p",   "wb"))
